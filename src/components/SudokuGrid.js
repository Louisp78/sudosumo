...
renderSquare(i){
    ...
    if (hint != null && hint.includes({x: x, y: y})){
        ...
        return <Box key={i} value={this.props.values[i]} onClick={() => this.props.handleClick(i)} hintMode={true}/>;
    }
    else{
        return <Box key={i} value={this.props.values[i]} onClick={() => this.props.handleClick(i)} hintMode={false}/>;
    }
}
